...
describe("createClaudeRunner", () => {
  test("R3: runs claude -p with the prompt and model in the workspace, HOME and CLAUDE_CONFIG_DIR redirected into it", async () => {
    ...
    const _result = await runner.run({ prompt: "review this session", workspaceDir })
 ...
 */
const writeHangingMsb = (workspaceDir: string): string => {
  const script = join(workspaceDir, "hanging-msb.sh")
  // Stands in for an `msb` that wedges instead of honouring its own --timeout: a VM that
  // will not boot, an image pull that stalls. The parent runner is the only thing that can
  // reap it, which is exactly what this shim is here to prove.
  writeFileSync(script, ["#!/usr/bin/env bash", "sleep 60", "exit 0"].join("\n"))
  chmodSync(script, 0o755)
  return script
}
...
describe("createMsbWrappedRunner", () => {
  test("MR1: wraps opencode in `msb run`, mounts the workspace, sets XDG inside the VM", async () => {
    ...
    // Model and prompt are both shell-quoted: the model comes straight off the analyze
    // request body, and this string is evaluated by a shell inside the guest.
  ...
  test("MR1b: the guest is handed the opencode credential, and nothing else from the host", () => {
    ...
    expect(flags).toContain("OPENCODE_API_KEY=oc-key")
    // msb forwards only what these flags name, so anything else the server holds -- its JWT
    // secret, its database url -- must not appear.
    expect(flags.filter((flag) => flag.startsWith("OPENCODE_API_KEY="))).toHaveLength(1)

    const withoutKey = buildMsbArgs({
      model: "x",
      timeoutMs: 60_000,
      image: "node:22-slim",
      msbSnapshot: undefined,
    ...
    expect(withoutKey.join(" ")).not.toContain("OPENCODE_API_KEY")
  })
  ...
  test("MR5: the inner command closes stdin to /dev/null — opencode blocks on an open stdin pipe", () => {
    ...
    // Snapshot variant: the npm bootstrap is gone, but the shell is not. A redirect is
    // only a redirect when a shell evaluates it -- passing `</dev/null` as a bare argv
    // token hands opencode a junk positional and leaves stdin open, which is the hang this
    // test exists to prevent. So both variants go through `sh -lc`, and both are asserted.
    const snapshotDashIdx = argsSnapshot.indexOf("--")
    const snapshotInner = argsSnapshot.slice(snapshotDashIdx + 1)
    expect(snapshotInner[0]).toBe("sh")
    const snapshotCommand = snapshotInner[snapshotInner.indexOf("-lc") + 1] ?? ""
    expect(snapshotCommand.trimEnd().endsWith("</dev/null")).toBe(true)
    expect(snapshotCommand).not.toContain("npm i -g")
  })
  ...
  test("MR8: a wedged msb is reaped by the parent timeout, so the job always settles", async () => {
    // Regression: the msb runner once hand-rolled its own spawn/close path and omitted the
    // timeout -> SIGTERM -> SIGKILL escalation that every other runner had. msb is the
    // production default for the opencode lane, so a wedged process was never killed, the
    // promise never settled, and the run held one of the registry's concurrency slots for
    // the lifetime of the server.
    const workspaceDir = mkdtempSync(join(tmpdir(), "samskara-msb-hang-"))
    const runner = createMsbWrappedRunner({
      model: "x",
      timeoutMs: 1_000,
      log: log(),
      msbBin: writeHangingMsb(workspaceDir),
    })
    ...
    // Reaped on the runner's own clock, nowhere near the shim's 60s sleep.
    expect(Date.now() - started).toBeLessThan(20_000)
  }, 30_000)

  test("MR9: terminateHarnessChildren kills a live run, and the run rejects", async () => {
    // What the server calls on SIGTERM. Its whole value is that the promise settles, because
    // each pipeline removes its workspace in the `finally` that rejection reaches.
    const workspaceDir = mkdtempSync(join(tmpdir(), "samskara-reap-"))
    const runner = createMsbWrappedRunner({
      model: "x",
      timeoutMs: 60_000,
      log: log(),
    ...
    const running = runner.run({ prompt: "p", workspaceDir })

    // The child has to be spawned before it can be reaped.
    await new Promise((resolve) => setTimeout(resolve, 200))
    expect(terminateHarnessChildren()).toBe(1)
    await expect(running).rejects.toMatchObject({ name: "HarnessRunnerError" })

    // Nothing left to reap once it has settled.
    expect(terminateHarnessChildren()).toBe(0)
  }, 30_000)
})
