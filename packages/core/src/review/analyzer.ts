...
const countEvents = (acc: Accumulator, event: ReviewEvent): void => {
  if (event.kind === "userMessage") {
    ...
    if (acc.failuresSincePrompt >= FAILURES_BEFORE_CORRECTION) {
      acc.userPromptsAfterFailures += 1
      acc.correctionSeqs.push(event.seq)
    }
    // A prompt with no intervening work is a re-prompt: the previous prompt did not stick.
    if (acc.userPrompts > 0 && acc.eventsSincePrompt === 0) {
      acc.rapidReprompts += 1
      acc.repromptSeqs.push(event.seq)
    }
    acc.userPrompts += 1
...
const outcomeOf = (
  ...
  if (friction === "high") return "struggled"
  return "productive"
}
...
export const heuristicSessionAnalyzer: SessionAnalyzer = {
  ...
  analyze({ sessionId, events, now }: ReviewInput): SessionReview {
    ...
    const signals: ReviewSignals = {
      ...
      correctionSeqs: acc.correctionSeqs,
      repromptSeqs: acc.repromptSeqs,
