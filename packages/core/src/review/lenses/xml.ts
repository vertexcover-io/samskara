 ...
 * - `entry` — one timeline entry. Attributes: `id`, `kind`, `from-seq`, `to-seq`, `tags`
 *   (comma-separated), and optionally `message-ids` (comma-separated) instead of the child
 *   container. Children: `title`, `summary`, `message-ids`. Kebab-case attributes have
 *   camelCase aliases (`fromSeq`, …). Tracks are derived from the seq range server-side, so
 *   there is no `tracks` attribute to get wrong.
...
<review outcome="shipped" friction="none" model="?" harness="?">
  ...
  <timeline>
    <!-- ${EXAMPLE_NOTE}
    <entry id="explore-phase" kind="phase" from-seq="0" to-seq="3" tags="exploration">
  ...
  text.replace(/&(amp|lt|gt|quot|apos|#\d+|#x[0-9a-fA-F]+);/g, (match, entity: string) => {
    switch (entity) {
      ...
      default: {
        ...
        // `String.fromCodePoint` throws a RangeError above U+10FFFF (and on surrogates),
        // and this text comes from a model. A throw here escapes the whole parser, so an
        // out-of-range entity is left as written instead — malformation is healed or
        // named, never fatal.
...
/**
 * The longest run a single tag may occupy before its first `>`. A tag in this contract is a
 * short element name plus a few attributes; anything longer is not a tag, and treating it as
 * one is what let the tokenizer degrade (see below).
 */
...
const tokenize = (xml: string): Token[] => {
  const tokens: Token[] = []
  let index = 0
  // `TAG_RE` is anchored at each `<` and, given no `>` to stop at, backtracks to the end of
  // the document before failing -- once per `<`. On model output like `"<a".repeat(n)` that
  // is quadratic: 100 KB blocked this (single-threaded) process for over two minutes. The
  // position of the next `>` is tracked instead, so a `<` that cannot begin a tag costs one
  // comparison, and the regex only runs when a plausible tag end is actually in reach.
  let nextGt = xml.indexOf(">")
  while (index < xml.length) {
    ...
    if (nextGt !== -1 && nextGt < lt) nextGt = xml.indexOf(">", lt)
    const commentStart = xml.startsWith("<!--", lt)
    if (!commentStart && (nextGt === -1 || nextGt - lt > MAX_TAG_CHARS)) {
      // No reachable `>`: nothing from here on can close a tag, so the rest is text.
      if (nextGt === -1) {
        tokens.push({ kind: "text", text: xml.slice(index) })
        break
      }
...
const balance = (tokens: ReadonlyArray<Token>, healed: string[]): Token[] => {
  const out: Token[] = []
  const stack: string[] = []
  const closeOf = (name: string): Token => {
...
type TimelineEntryInput = Extract<
  AiReviewPayloadInput["lenses"][number],
  { lens: "timeline" }
>["entries"][number]
...
// --- section collection, lens assembly and verdict ------------------------------------------

type CollectedSections = {
  readonly summary: string | undefined
  readonly selfCounts: ReviewCounts | undefined
  /** Sections found directly under root, plus any hoisted out of a legacy <lenses> wrapper. */
  readonly sections: ReadonlyMap<SectionTag, Element>
}

const collectSections = (root: Element, healed: string[]): CollectedSections => {
  ...
  for (const child of root.children) {
    ...
    if (child.name === "counts" && selfCounts === undefined) {
      selfCounts = countsFrom(child)
      if (selfCounts === undefined) healed.push("malformed <counts>")
      continue
    }
    if ((SECTION_TAGS as readonly string[]).includes(child.name)) {
      placeSection(child)
      continue
    }
  ...
  return { summary, selfCounts, sections }
}

const entriesFrom = (section: Element, healed: string[]): TimelineEntryInput[] => {
...
const learningsFrom = (
  section: Element,
  tag: Exclude<SectionTag, "timeline">,
  healed: string[],
): LearningInput[] => {
 ...
 * still fails validation in `parseReviewXml`; missing learnings sections parse as empty and
 * surface through `recovered` plus the counts-mismatch `partial`.
 */
const lensesFrom = (
  sections: ReadonlyMap<SectionTag, Element>,
  healed: string[],
): AiReviewPayloadInput["lenses"] => {
  ...
  for (const tag of SECTION_TAGS) {
    ...
    if (tag === "timeline") {
      lenses.push({ lens: "timeline", entries: entriesFrom(section, healed) })
      continue
    }
    lenses.push({
      lens: tag,
      learnings: learningsFrom(section, tag, healed),
    } as AiReviewPayloadInput["lenses"][number])
  }
  return lenses
}

const normalizeEnum = (
  key: "outcome" | "friction",
...
type Verdict = Pick<AiReviewPayloadInput, "model" | "harness" | "outcome" | "friction">

const verdictFrom = (root: Element, healed: string[]): Verdict => {
  ...
  return {
    ...
    // Enum-shaped strings: zod is the authority that they are real enum values, after
    // synonym normalization — a model writing friction="overcome" must not cost the whole
    // review.
    outcome: normalizeEnum(
      "outcome",
      root.attrs.outcome,
      healed,
    ) as AiReviewPayloadInput["outcome"],
    friction: normalizeEnum(
      "friction",
      root.attrs.friction,
      healed,
    ) as AiReviewPayloadInput["friction"],
  }
}
...
/**
 * The largest deliverable this parser will look at. The contract's own caps -- a 600-char
 * summary, ~8 timeline entries, a handful of learnings -- fit in a few tens of KB, so this is
 * generous by an order of magnitude. It exists because the input is written by the agent
 * under review inside the workspace, and both the file read and the stdout fallback are
 * otherwise unbounded.
 */
export const MAX_REVIEW_XML_CHARS = 512 * 1024

/**
 * Parses one reviewer deliverable. Total function by contract: malformation is healed or
 * named, never thrown -- callers treat a throw as a harness failure, which misreports a
 * model that merely emitted bad XML. The body is wrapped so an unforeseen throw still
 * becomes a named parse error.
 */
export const parseReviewXml = (text: string): ReviewXmlResult => {
  try {
    return parseReviewXmlUnguarded(text)
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    return { ok: false, error: `parser error: ${message}`, recovered: [] }
  }
}

const parseReviewXmlUnguarded = (source: string): ReviewXmlResult => {
  const healed: string[] = []
  const text = source.length > MAX_REVIEW_XML_CHARS ? source.slice(0, MAX_REVIEW_XML_CHARS) : source
  if (text !== source) {
    healed.push(`truncated deliverable to ${MAX_REVIEW_XML_CHARS} chars`)
  }
  const noRoot = `no <${REVIEW_XML_ROOT}> element found in the output`

  const salvaged = salvageXml(text)
  if (salvaged === null) return fail(noRoot, healed)
  ...
  if (root === null) return fail(noRoot, healed)

  const { summary, selfCounts, sections } = collectSections(root, healed)
  const lenses = lensesFrom(sections, healed)
  ...
  if (timelineLens === undefined || timelineLens.entries.length === 0) {
    ...
    return fail(`the timeline needs at least one entry — ${why}`, healed, selfCounts)
  }

  const input: AiReviewPayloadInput = {
    analyzer: "ai-v1",
    ...verdictFrom(root, healed),
    summary: summary ?? "",
    lenses,
  }
  ...
  if (!validated.success) {
    const issues = validated.error.issues
      ...
      .join("; ")
    return fail(`schema validation failed: ${issues}`, healed, selfCounts)
  }
  ...
  return succeed(
    partial === undefined ? validated.data : { ...validated.data, partial },
    healed,
    selfCounts,
  )
}
