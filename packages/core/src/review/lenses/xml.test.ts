...
describe("root attribute synonym healing", () => {
  ...
  test("X-DOS1: a long run of unterminated tags parses fast instead of blocking the event loop", () => {
    // The tokenizer anchored a regex at each `<` and, with no `>` to stop at, backtracked to
    // the end of the document before failing -- once per `<`. Measured before the fix: 100 KB
    // of `"<a"` blocked this process for 145 s, cleanly quadratic. The server is
    // single-threaded and this input is written by the agent under review.
    const hostile = `<review outcome="shipped" friction="none">${"<a".repeat(500_000)}`
    const startedAt = Date.now()
    const result = parseReviewXml(hostile)
    expect(Date.now() - startedAt).toBeLessThan(2_000)
    expect(result.ok).toBe(false)
  })

  test("X-DOS2: an out-of-range numeric entity is healed, not thrown", () => {
    // `String.fromCodePoint` throws above U+10FFFF. The throw escaped the parser entirely and
    // the job settled as `harnessFailed`, blaming the harness for the model's bad XML.
    expect(() =>
      parseReviewXml(
        '<review outcome="shipped" friction="none"><summary>cost 10&#99999999;</summary></review>',
      ),
    ).not.toThrow()
  })

  test("X-DOS3: a deeply nested document returns a parse error rather than overflowing", () => {
    const deep = `<review outcome="shipped" friction="none">${"<entry>".repeat(60_000)}`
    expect(() => parseReviewXml(deep)).not.toThrow()
    expect(parseReviewXml(deep).ok).toBe(false)
  })

  test("X-DOS4: a deliverable past the cap is truncated and the truncation is reported", () => {
    const padded = `<review outcome="shipped" friction="none"><summary>${"x".repeat(
      MAX_REVIEW_XML_CHARS,
    )}</summary></review>`
    const result = parseReviewXml(padded)
    expect(result.recovered).toContain(`truncated deliverable to ${MAX_REVIEW_XML_CHARS} chars`)
  })
})

describe("tracks are not the model's to name", () => {
  const timelineTracks = (xml: string): ReadonlyArray<string> | null => {
    const parsed = parseReviewXml(xml)
    if (!parsed.ok) return null
    const timeline = parsed.value.lenses.find((lens) => lens.lens === "timeline")
    if (timeline === undefined || !("entries" in timeline)) return null
    return timeline.entries[0]?.tracks ?? null
  }

  test("X-TRACKS1: a tracks attribute is ignored, whatever delimiter it used", () => {
    expect(timelineTracks(filledXml.replace('tracks="main"', 'tracks="agent:a agent:b"'))).toEqual(
      [],
    )
    expect(timelineTracks(filledXml.replace('tracks="main"', 'tracks="agent:a,agent:b"'))).toEqual(
      [],
  ...
  test("X-TRACKS2: the template no longer offers a tracks attribute to fill in", () => {
    expect(reviewXmlTemplate()).not.toContain("tracks=")
  })
})
