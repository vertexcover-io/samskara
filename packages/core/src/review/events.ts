...
/**
 * Both harnesses' write tools. Claude names them in CamelCase and passes `file_path`;
 * opencode names them lowercase and passes `filePath` (see the collector's `WRITE_TOOLS`).
 * A name missing here costs the whole rework signal for that harness, silently.
 */
  ...
  for (const message of messages) {
    switch (message.msgType) {
      ...
      case "toolCall": {
        ...
        const input = message.details.input as {
          file_path?: string
          filePath?: string
        } | null
