...
const seqsWhere = (
  ...
  pred: (event: ReviewEvent) => boolean,
): number[] => events.filter(pred).map((event) => event.seq)
...
export const extractLearnings = (
  ...
  if (signals.userPromptsAfterFailures >= 1) {
    human.push({
      ...
      evidence: evidenceFrom(signals.correctionSeqs, () => "prompt sent after repeated failures"),
  ...
  if (signals.rapidReprompts >= RAPID_REPROMPT_THRESHOLD) {
    human.push({
      ...
      evidence: evidenceFrom(signals.repromptSeqs, () => "re-prompt with no intervening work"),
