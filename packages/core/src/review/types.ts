...
export type ReviewSignals = {
  ...
  /** Seqs of the prompts that qualified as corrections — the analyzer's own record,
   * so nothing downstream has to re-decide the rule to point at the evidence. */
  readonly correctionSeqs: ReadonlyArray<number>
  /** Seqs of the prompts counted by `rapidReprompts`, so evidence and count cannot disagree. */
  readonly repromptSeqs: ReadonlyArray<number>
