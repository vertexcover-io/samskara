...
import type { Artifact, TimelineRecord } from "./records.js"
...
const Prose = ({ body }: { body: string }) =>
  body === "" ? (
    ...
    <>
      {body.split("\n\n").map((paragraph, index) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: paragraphs have no stable identity
        <p key={index} className="max-w-[72ch] whitespace-pre-wrap [&+&]:mt-2">
          {paragraph}
        </p>
      ))}
    </>
...
const Thinking = ({ thinking }: { thinking: string }) => (
  <details className="my-2">
    ...
    <div className="mt-2 max-w-[72ch] whitespace-pre-wrap border-l-2 border-dashed border-rule px-3 py-2 italic text-ink-soft">
      {thinking}
    </div>
  </details>
)
...
const Exhibit = ({ artifact }: { artifact: Artifact }) => (
  <div className="flex max-w-[74ch] items-start gap-3 rounded-xs border border-rule bg-panel-2 px-3 py-3">
    <span
      aria-hidden="true"
...
const Shell = ({ children }: { children: ReactNode }) => (
  <article className="relative py-3">{children}</article>
)
...
export const RecordItem = ({ record, showTools = true, annex }: Props) => {
  if (record.kind === "prompt") {
    return (
      <Shell>
        <Head actor={record.actor} kind="Request" timestamp={record.timestamp} />
        <div className="mt-2 max-w-[74ch] rounded-xs border border-rule border-l-2 border-l-ink bg-panel-2 px-3 py-3">
  ...
  if (record.kind === "assistant") {
    return (
      <Shell>
        <Head actor={record.actor} timestamp={record.timestamp} model={record.model} />
        <div className="mt-2 max-w-[74ch] rounded-xs border border-rule bg-panel-2 px-3 py-3">
