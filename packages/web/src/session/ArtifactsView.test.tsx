...
test("S54: the preview frame is not sandboxed, so a captured report can load its own media", () => {
...
test("S53: the filename filter narrows the tree to matching files and leaves the rest out", async () => {
  const user = userEvent.setup()
  render(
    <TestRouter initialEntries={["/s/1?tab=artifacts"]}>
      <ArtifactsView artifacts={ARTIFACTS} />
  ...
  await user.type(screen.getByRole("searchbox", { name: /filter by filename/i }), "idempotency")

  const rows = within(screen.getByRole("list", { name: /filed artifacts/i }))
    .getAllByRole("button")
    .map((button) => button.textContent ?? "")

  expect(rows.some((row) => row.includes("idempotency-design.md"))).toBe(true)
  expect(rows.some((row) => row.includes("0007_add_source_uid.sql"))).toBe(false)
})

test("S53: a filter that matches nothing keeps its own controls on screen rather than stranding the reader", async () => {
  const user = userEvent.setup()
  render(
    <TestRouter initialEntries={["/s/1?tab=artifacts"]}>
      <ArtifactsView artifacts={ARTIFACTS} />
...
test("S53: a session that filed nothing reads as empty rather than as a filter that matched nothing", () => {
  render(
    <TestRouter initialEntries={["/s/1?tab=artifacts"]}>
      <ArtifactsView artifacts={[]} />
    </TestRouter>,
  ...
  expect(screen.getByText(/no artifacts were filed in this session/i)).toBeInTheDocument()
  expect(screen.queryByRole("searchbox", { name: /filter by filename/i })).not.toBeInTheDocument()
})

test("S53: the wrap toggle is off by default and switches the body between scrolling and wrapping", async () => {
  const user = userEvent.setup()
  render(
    <TestRouter initialEntries={["/s/1?tab=artifacts"]}>
      <ArtifactsView artifacts={ARTIFACTS} />
  ...
  const toggle = screen.getByRole("checkbox", { name: /wrap lines/i })
