    ...
    <div className="max-w-full rounded-xs border border-rule border-t-2 border-t-custody bg-panel">
      <div className="flex flex-wrap items-start gap-3 border-b border-rule bg-custody/5 px-3 py-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 font-mono text-[0.6875rem] text-faded">
      ...
      <div className="flex flex-wrap items-center gap-2 px-3 py-2">
        <button
          ...
          onClick={(event) => onOpen(agent.agentId, event.currentTarget)}
        ...
        >
          {open ? "Branch open" : "Open annex"} <span aria-hidden="true">{open ? "▾" : "▸"}</span>
        </button>
        {open ? (
          <button
            type="button"
            onClick={onExit}
            className="inline-flex items-center gap-2 rounded-xs border border-rule bg-panel-2 px-3 py-2 text-[0.75rem] font-semibold transition-colors hover:border-ink-soft"
          >
            Return to the main record
          </button>
        ) : null}
      </div>
