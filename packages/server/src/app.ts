...
type Deps = {
  ...
  /** Test seam for the credential probe behind the same endpoint. */
...
const defaultAiReviewRunner = (env: Env, log: pino.Logger): HarnessRunner => {
  ...
  return {
    run: (input) => {
      const byHarness: Readonly<Record<ReviewHarness, HarnessRunner>> = { opencode, claude }
      const chosen: HarnessRunner = byHarness[input.harness ?? env.aiReviewHarness]
      return chosen.run(input)
    },
  }
}
  ...
  const app = new Hono<{ Variables: Variables }>()
    ...
    .route(
      ...
      reviewRoutes({
        ...
        aiReviewRunner,
        aiReviewJobs,
        ...(deps.credentialEnv === undefined ? {} : { credentialEnv: deps.credentialEnv }),
      }),
    )
    // Its own base: a static /api/sessions/... path would be captured by the `/:id` detail route.
    .route(
      "/api/reviewer-options",
      reviewerOptionsRoutes({ db, env, commandExists, credentialEnv: deps.credentialEnv }),
    )
