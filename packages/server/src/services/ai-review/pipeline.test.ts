...
describe.skipIf(!dockerAvailable())("runAiReview", () => {
  let db: Db
  ...
  const depsWith = (
    ...
    mkdtemp: mkdtempSpy,
    // Credentials come from the environment now, never a staged file; tests declare one
    // rather than depending on whatever the machine running them happens to export.
    credentialEnv: { OPENCODE_API_KEY: "oc-key", CLAUDE_CODE_OAUTH_TOKEN: "cc-token" },
    ...overrides,
  })
  ...
  test("P17: AI_REVIEW_HARNESS=claude persists the claude harness on the review instead of a hardcoded one", async () => {
  ...
  test("P19: a harness with no credential is refused before the runner is touched", async () => {
    let calls = 0
    ...
    const result = await runAiReview(
      // Configured for opencode, asked for claude, and only opencode has a key.
    ...
    expect(calls).toBe(0)
    expect(await db.select().from(sessionReviews)).toHaveLength(0)
  })

  test("P21: the pipeline reports milestones through onMilestone, in order", async () => {
    const seen: string[] = []
    const { runner } = fakeRunner(() => okBehaviour())
    const result = await runAiReview(
    ...
    // names and their order are the contract the CLI's progress line reads.
    expect(seen).toContain("workspace_ready")
    expect(seen).toContain("harness_spawning")
    expect(seen.indexOf("workspace_ready")).toBeLessThan(seen.indexOf("harness_spawning"))
    expect(seen.at(-1)).toBe("persisted")
  })
  ...
  test("P2c: a learning write that fails rolls the review row back", async () => {
    // Nothing in a valid payload can make upsertLearning fail, so the failure is injected at
    // the only layer that can refuse the write unconditionally.
    await db.execute(
      sql`create or replace function fail_learning() returns trigger language plpgsql as $$
        begin raise exception 'learning write refused'; end $$`,
    )
    await db.execute(
      sql`create trigger fail_learning_trigger before insert on learnings
        for each row execute function fail_learning()`,
    )
    try {
      const { runner } = fakeRunner(() => okBehaviour())
  ...
  test("P20: a reviewer-written number too large for int4 still persists", async () => {
    // The title is the model's own prose, so the number in it is whatever it wrote. The
    // title-magnitude comparison casts that number in SQL; an int4 cast raises on anything
    // above 2147483647 and takes the whole persist transaction down with it.
    const payload = validPayload(sessionExport.index)
    const huge = {
      ...payload,
      lenses: payload.lenses.map((lens) =>
        lens.lens === "agentLearnings"
          ? {
              ...lens,
              learnings: lens.learnings.map((learning) => ({
                ...learning,
                title: "Bash failed 9999999999 times in a row",
              })),
            }
          : lens,
      ),
    } as typeof payload
    const { runner } = fakeRunner(() => ({ stdout: "ok", file: xmlOf(huge) }))
    ...
    expect(titles).toContain("Bash failed 9999999999 times in a row")
  })
  ...
  test("P18: tracks are derived from the seq range, so a track the model invents never lands", async () => {
    const base = validPayload(sessionExport.index)
    const timeline = base.lenses[0] as {
      lens: "timeline"
      entries: ReadonlyArray<FixtureTimelineEntry>
    }
    const invented = {
      ...base,
      lenses: [
        {
          ...
          entries: timeline.entries.map((entry) => ({
            ...entry,
            tracks: ["agent:ghost agent:other"],
          })),
        },
        ...base.lenses.slice(1),
      ],
    ...
    const lenses = signals.lenses as ReadonlyArray<{
      lens: string
      entries?: ReadonlyArray<{ tracks: ReadonlyArray<string> }>
    }>
