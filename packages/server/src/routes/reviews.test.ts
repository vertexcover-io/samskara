...
describe.skipIf(!dockerAvailable())("reviews and learnings", () => {
  let db: Db
  ...
  test("V3: review writes learnings for both audiences, deduped by fingerprint", async () => {
    ...
    expect(body.learnings.some((l) => l.audience === "agent" && l.category === "tool-retry")).toBe(
      true,
    )
  ...
  test("V14: a viewer cannot run the static review — it writes reviews and learnings", async () => {
    await seedErrorLoop()
    const [stranger] = await db
      .insert(users)
      .values({ githubId: 4, githubLogin: "viewer-static" })
      .returning({ id: users.id })
    await db
      .insert(userProjectGrant)
      .values({ userId: stranger?.id as string, projectId, scope: "viewer" })
    const strangerToken = await signToken(env, { sub: stranger?.id as string, aud: "web" })

    const res = await buildApp(db, env).request(`/api/sessions/${sessionId}/review`, {
      method: "POST",
      headers: { cookie: `session=${strangerToken}` },
    })
    expect(res.status).toBe(403)
  ...
  test("V15: a bad learnings filter is a 400, not a 500", async () => {
    expect((await request("/api/learnings?audience=bogus")).status).toBe(400)
    expect((await request("/api/learnings?status=bogus")).status).toBe(400)
    // Reaches a uuid column; unvalidated it surfaced as a Postgres 22P02 -> 500.
    expect((await request("/api/learnings?projectId=not-a-uuid")).status).toBe(400)
    expect((await request("/api/learnings")).status).toBe(200)
  })

  test("V16: a non-uuid learning id is a 404, not a database error", async () => {
    const res = await request("/api/learnings/not-a-uuid/status", {
      method: "PATCH",
      body: { status: "accepted" },
    })
    expect(res.status).toBe(404)
