...
const titleMagnitude = (title: SQLWrapper | string): SQL =>
  // ::bigint, not ::int: the title is the reviewer's own prose, so the number in it is
  // whatever the model wrote. "Bash failed 9999999999 times" overflows an int4 and the cast
  // raises, which aborts the whole persist transaction -- review and learnings both -- after
  // a full harness run. Digits are also cut to 18 so a pathological run of them cannot
  // overflow int8 in turn.
  sql`(select coalesce(max(substring(digits[1] from 1 for 18)::bigint), 0) from regexp_matches(${title}, '(\\d+)', 'g') as digits)`
...
export const upsertLearning = async (
  ...
  /**
   * The session the occurrence is attributed to. Callers know it -- they just wrote the
   * review for it -- so it is passed rather than read back: the lookup it replaces was an
   * unprojected select of the review row, whose `signals` jsonb carries the milestone
   * ledger, the capped agent log and the reviewer transcript, re-read once per learning.
   */
  sessionId: string,
): Promise<LearningRow> => {
  ...
  // Both writes belong together, so a caller already inside a transaction lends us its own
  // rather than opening a savepoint per learning.
  const inTransaction = async (tx: Querier): Promise<LearningRow> => {
    ...
    const alreadyCounted = sql`exists (
      ...
      where ${learningSessions.learningId} = ${learnings.id}
        and ${learningSessions.sessionId} = ${sessionId}
    )`
    const [row] = await tx
      .insert(learnings)
      .values(values)
    ...
    await tx
      ...
      .values({ learningId: row.id, sessionId })
      .onConflictDoNothing()
    return row
  }
  return isTransaction(db) ? inTransaction(db) : db.transaction(inTransaction)
}

/** A Drizzle transaction exposes `rollback`; the top-level db handle does not. */
const isTransaction = (db: Querier): boolean =>
