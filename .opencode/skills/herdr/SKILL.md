...
Waits are one-shot, not subscriptions. A wait returns on the first settled state and then stops watching; an agent that resumes `working` (after you unblock it, or on its own) and later finishes notifies nobody. After every wait return — especially `blocked` — issue a fresh wait once the agent is working again, and keep re-arming until the work's own completion criterion is met (the artifact it was asked to produce, not merely an `idle` pane). `herdr notification` does not help here: it only displays a desktop notification to the human; it cannot subscribe you to agent events.
